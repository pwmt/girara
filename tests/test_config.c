...
START_TEST(test_config_parse) {
  ...
  fail_unless(session != NULL, "Failed to create girara session.");
  ...
  char* ptr = NULL;
  fail_unless(girara_setting_get(session, "test1", &ptr), "Failed to get setting 'test1'.");
  fail_unless(g_strcmp0(ptr, "config-string") == 0, "Value of 'test1' doesn't match (got: %s, expected: %s",
      ptr, "config-string");
  g_free(ptr);

  int real_val = 0;
  fail_unless(girara_setting_get(session, "test2", &real_val), "Failed to get setting 'test1'.");
